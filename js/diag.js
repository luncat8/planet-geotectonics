...
var Diag = {
	check: function (s) {
		var g = s.grid, qErr = 0, rigid = 0, finite = 1, scratch = s.scratch;
		...
		for (var i = 0; i < s.n; i++) {
			var b = i * 3;
			DiagQuat.rotate(scratch, 0, s.q, s.plate[i] * 4, s.body, b);
			var err = Math.hypot(scratch[0] - s.world[b], scratch[1] - s.world[b + 1], scratch[2] - s.world[b + 2]);
			if (err > rigid) rigid = err;
			if (!Number.isFinite(s.world[b] + s.world[b + 1] + s.world[b + 2] + s.body[b] + s.hFel[i] + s.hMaf[i] + s.age[i])) finite = 0;
		}
		for (var c = 0; c < g.V; c++) {
			if (s.owner[c] < 0) continue;
			var b = c * 3;
			if (!Number.isFinite(s.vel[b] + s.uMantle[b] + s.z[c] + s.ext[c])) finite = 0;
		}
