...
Aliasing (7 % of cells with 2 columns, 7 % with 0 but covered) is harmless: the non-owner
column is still tracked and still updated by column kernels.
...
ω_target = M⁻¹ b ;   ω ← ω + (ω_target − ω) · min(1, dt / τ_ω),  τ_ω = 1 Myr ;   |ω| R ≤ vMax
```
...
| collision resistance | both sides of a C–C convergent edge | `− vColl · max(0, −relN)/vRef · n_ij` with the previous frame's `relN` (explicit coupling is stable at τ_ω = 1 Myr) |
...
```
e_i      = kEro · max(0, z_i) · (1 + 2·slope_i/slopeRef) · dt        removed from hSed, then hFel, then hMaf
...
No fluid transport at 10–50 km cells: source, discharge and trap fall in one or two cells, so a
potential is accumulated locally where the geologic factory operates, with a 1-cell blur at
...
```
rGap 0.75   rContact 0.6   K 3   hRiftBreakup 15e3   hOceanic 8e3   hCollapse 50e3
ε_hi 2 mm/yr  ε_lo 1 mm/yr  vRef 5 cm/yr  vMax 20 cm/yr  vSuture 3 mm/yr  τ_ω 1 Myr
...
kEro 0.05/Myr   slopeRef 0.01   δ 20 m   kPlacer 0.2
